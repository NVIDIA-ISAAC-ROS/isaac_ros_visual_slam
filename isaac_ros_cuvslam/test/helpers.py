...
def run_cuvslam_rgbd_from_bag(namespace: str,
    ...
    # At normal speed, this short recording can finish while cuVSLAM is still
    # initializing. Slower playback leaves enough frames to verify odometry.
    cmd = ['ros2', 'bag', 'play', str(bag_path), '--rate', '0.25', '--remap']
    cmd.extend([
    ...
    # Start the short RGBD bag after the cuVSLAM node and the test subscriber have
    # joined the ROS graph. Otherwise playback can consume all frames before either
    # subscriber is ready on slower runners.
    rosbag_play = lut.TimerAction(
        period=5.0,
        actions=[lut.ExecuteProcess(cmd=cmd, output='screen')],
    )
